...
function ResponseReplyCard(props: Props) {
	const { isPublicationPost, response, reply, draftId } = props;
		...
		replaceFn: (_autolinker: any, match: any) => {
			switch (match.getType()) {
				case 'twitter':
					...
					return `<div  class="user-mention" target="_blank" rel="noopener">@${username}</div>`;
				default:
					return null;
				// case 'url':
											...
											<a
												...
												href={
													reply.author && !reply.author.isDeactivated
														? `${isPublicationPost ? 'https://hashnode.com' : ''}/@${
